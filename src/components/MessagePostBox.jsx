...
export default function MessagePostBox({ placeholder = "What're your thoughts on $TSLA?", onPost, initialPredictionOpen = false, blankSymbol = false }) {
  ...
  const [showPredictionModal, setShowPredictionModal] = useState(initialPredictionOpen)
  const [predSymbol, setPredSymbol] = useState(blankSymbol ? '' : 'PLTR')
                      ...
                      {blankSymbol ? (
                        <div className="relative">
                          ...
                          <input
                            type="text"
                            value={predSymbol}
                            onChange={(e) => setPredSymbol(e.target.value.toUpperCase().replace(/[^A-Z]/g, ''))}
                            placeholder="TSLA"
                            className="w-full pl-5 pr-2.5 py-2 rounded-lg bg-surface border border-border text-text text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary"
                          />
                        </div>
                      ) : (
