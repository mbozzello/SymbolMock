...
/**
 * iOS-styled share sheet modal.
 * Props:
 *   open      – boolean
 *   onClose   – callback
 *   lightMode – boolean (default false)
 */
export default function IOSShareSheet({ open, onClose, lightMode = false }) {
  ...
  if (xComposeOpen) {
    return (
      <div className={clsx('absolute inset-0 z-50 flex flex-col', L ? 'bg-white' : 'bg-black')} style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "SF Pro Display", "Helvetica Neue", sans-serif' }}>
        <div className={clsx('shrink-0 flex items-center justify-between px-4 py-3 border-b', L ? 'border-gray-200' : 'border-white/10')}>
          <button type="button" onClick={handleClose} className={clsx('text-sm', L ? 'text-gray-500' : 'text-white/70')}>Cancel</button>
        ...
        <div className="flex-1 overflow-y-auto px-4 pt-4">
          <div className="flex gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className={clsx('mt-3 rounded-2xl border overflow-hidden', L ? 'border-gray-200 bg-gray-50' : 'border-white/10 bg-[#16181c]')}>
                <div className="flex items-center gap-2 p-3 pb-2">
                  <img src="/avatars/howard-lindzon.png" alt="" className="w-6 h-6 rounded-full object-cover" />
                  <span className={clsx('text-sm font-semibold', L ? 'text-gray-900' : 'text-white')}>@howardlindzon</span>
                ...
                <div className={clsx('relative mx-3 rounded-xl overflow-hidden border', L ? 'bg-gray-100 border-gray-200' : 'bg-black border-white/5')}>
                  <div className="absolute top-2 right-2 z-10">
                    <img src="/images/stocktwits-logo.png" alt="Stocktwits" className="h-5 opacity-60" />
                  </div>
                  <div className={clsx('w-full h-44 flex items-center justify-center relative', L ? 'bg-gray-100' : 'bg-[#0d1117]')}>
                    <svg className="w-full h-full p-4" viewBox="0 0 400 140" preserveAspectRatio="none">
                      {[0, 35, 70, 105, 140].map(y => (
                        <line key={y} x1="0" y1={y} x2="400" y2={y} stroke={L ? 'black' : 'white'} strokeOpacity="0.05" strokeWidth="0.5" />
                      ))}
                    ...
                    <div className="absolute top-3 left-3">
                      <div className={clsx('text-[10px]', L ? 'text-gray-400' : 'text-white/40')}>$HOOD</div>
                      <div className={clsx('text-sm font-bold tabular-nums', L ? 'text-gray-900' : 'text-white')}>$46.64</div>
                    </div>
                  </div>
                  <div className={clsx('flex items-center gap-2 px-3 py-2 border-t', L ? 'bg-gray-50 border-gray-200' : 'bg-[#0d1117] border-white/5')}>
                    <span className={clsx('text-xs', L ? 'text-gray-500' : 'text-white/50')}>Since post</span>
                    <div className={clsx('flex items-center gap-1.5 px-2 py-1 rounded-full', L ? 'bg-gray-200' : 'bg-white/10')}>
                      <img src="/images/logos/hood.png" alt="" className="w-4 h-4 rounded-full object-cover" />
                      <span className={clsx('text-xs font-semibold', L ? 'text-gray-900' : 'text-white')}>$HOOD</span>
                ...
                <div className="p-3 pt-2">
                  <p className={clsx('text-[13px] leading-snug line-clamp-2', L ? 'text-gray-600' : 'text-white/60')}>$HOOD updated robinhood cheat sheet — The SuperApp Nobody's Pricing In...</p>
                  <p className={clsx('text-[11px] mt-1', L ? 'text-gray-400' : 'text-white/30')}>From stocktwits.com</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className={clsx('shrink-0 border-t', L ? 'border-gray-200' : 'border-white/10')}>
          ...
          <div className={clsx('flex items-center gap-4 px-4 py-2 border-t', L ? 'border-gray-200' : 'border-white/10')}>
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-end justify-center" role="dialog" aria-modal="true">
      ...
      <div
        className={clsx('relative w-full max-h-[80%] rounded-t-2xl overflow-hidden flex flex-col animate-[slideUp_0.25s_ease-out]', L ? 'bg-white' : 'bg-[#1c1c1e]')}
        style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "SF Pro Display", "Helvetica Neue", sans-serif' }}
      >
        <div className="flex justify-center pt-2 pb-1">
          <div className={clsx('w-9 h-1 rounded-full', L ? 'bg-gray-300' : 'bg-white/20')} />
        </div>

        <div className="flex items-center justify-between px-4 pb-3">
          <span className={clsx('text-base font-semibold', L ? 'text-gray-900' : 'text-white')}>Share to...</span>
          <button type="button" onClick={handleClose} className={clsx('w-7 h-7 rounded-full flex items-center justify-center', L ? 'bg-gray-200' : 'bg-white/10')}>
            <svg className={clsx('w-3.5 h-3.5', L ? 'text-gray-500' : 'text-white/60')} fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
          </button>
        </div>

        <div className="flex-1 overflow-y-auto">
          <div className={clsx('mx-4 mb-4 rounded-xl border overflow-hidden', L ? 'border-gray-200 bg-gray-50' : 'border-white/10 bg-white/5')}>
            <div className="flex items-start gap-3 p-3">
              <img src="/avatars/howard-lindzon.png" alt="" className="w-8 h-8 rounded-full object-cover shrink-0" />
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1">
                  <span className={clsx('text-xs font-semibold', L ? 'text-gray-900' : 'text-white')}>@howardlindzon</span>
                ...
                <p className={clsx('text-[13px] mt-1 leading-snug', L ? 'text-gray-600' : 'text-white/70')}>$HOOD updated robinhood cheat sheet</p>
                <div className={clsx('flex items-center gap-3 mt-1.5 text-[10px]', L ? 'text-gray-400' : 'text-white/30')}>
                  <span>▲ 7</span>
                  <span>💬 1 comment</span>
                </div>
              </div>
              <div className={clsx('shrink-0 w-16 h-16 rounded-lg overflow-hidden border', L ? 'bg-gray-100 border-gray-200' : 'bg-black border-white/10')}>
                <svg className="w-full h-full p-1" viewBox="0 0 100 60" preserveAspectRatio="none">
                  {[0,15,30,45,60].map(y => (
                    <line key={y} x1="0" y1={y} x2="100" y2={y} stroke={L ? 'black' : 'white'} strokeOpacity="0.05" strokeWidth="0.5" />
                  ))}
            ...
            <div className={clsx('px-3 pb-2 text-[10px]', L ? 'text-gray-400' : 'text-white/30')}>From stocktwits.com</div>
          </div>

          <p className={clsx('text-[11px] px-4 mb-3', L ? 'text-gray-400' : 'text-white/30')}>Your username stays hidden when you share outside of Stocktwits.</p>

          <div className="px-4 pb-4">
            <div className="flex items-center gap-3 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
              <button type="button" onClick={() => setXComposeOpen(true)} className="shrink-0 flex flex-col items-center gap-1.5">
                <div className={clsx('w-12 h-12 rounded-full flex items-center justify-center', L ? 'bg-gray-100' : 'bg-white')}>
                  <svg className="w-5 h-5 text-black" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                  </svg>
                </div>
                <span className={clsx('text-[10px]', L ? 'text-gray-500' : 'text-white/50')}>X</span>
              </button>
              <button type="button" className="shrink-0 flex flex-col items-center gap-1.5">
                ...
                <span className={clsx('text-[10px]', L ? 'text-gray-500' : 'text-white/50')}>Whatsapp</span>
              </button>
              <button type="button" className="shrink-0 flex flex-col items-center gap-1.5">
                ...
                <span className={clsx('text-[10px]', L ? 'text-gray-500' : 'text-white/50')}>Messages</span>
              </button>
              <button type="button" className="shrink-0 flex flex-col items-center gap-1.5">
                ...
                <span className={clsx('text-[10px] text-center leading-tight', L ? 'text-gray-500' : 'text-white/50')}>Instagram<br/>Stories</span>
              </button>
              <button type="button" className="shrink-0 flex flex-col items-center gap-1.5">
                ...
                <span className={clsx('text-[10px]', L ? 'text-gray-500' : 'text-white/50')}>Telegram</span>
              </button>
              <button type="button" className="shrink-0 flex flex-col items-center gap-1.5">
                <div className="w-12 h-12 rounded-full flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #833AB4, #FD1D1D, #F77737)' }}>
                ...
                <span className={clsx('text-[10px]', L ? 'text-gray-500' : 'text-white/50')}>Instagram</span>
              </button>
            </div>
          </div>

          <div className={clsx('mx-4 border-t', L ? 'border-gray-200' : 'border-white/10')} />
          ...
          <div className="flex items-start justify-around px-4 py-4">
            {[
              ...
              >
                <div className={clsx('w-12 h-12 rounded-full flex items-center justify-center', L ? 'bg-gray-100 text-gray-600' : 'bg-white/10 text-white/60')}>
                  {action.icon}
                </div>
                <span className={clsx('text-[10px] text-center leading-tight', L ? 'text-gray-500' : 'text-white/50')}>{action.label}</span>
              </button>
            ))}
          </div>
        </div>
